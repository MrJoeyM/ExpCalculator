...
public class Calculator {
	...
	public boolean isOperator(char _operator) {
	...
	public int compareCaculatorOperator(String _caculatorOperator1,
	...
	public ExStack infixToPosfix(ExStack infixStack) {
		...
		for (int i = 0; i < infixStack.size(); i++) // 注意遍历区间控制!!
		{
	...
	public String caculate(ExStack posfix) {
		...
		for (int i = 0; i < posfix.size(); i++) {
			if (isNumStr(posfixStr[i])) { // 是数字，进行压栈
				caculateStack.push(posfixStr[i]);
			} else if (isOperatorStr(posfixStr[i])) { // 是运算符，取出数字，进行计算
				if (caculateStack.size() < 2) // 栈中数字数目小于2，报错
				{
	...
	public static void main( String args[]){
		...
		System.out.println("请输入计算表达式(以-1退出):");
		String cal_str = in.nextLine();
		
		while( !"-1".equals(cal_str)){
			String result_str = ocal.caculateFromString(ocal.strFilter(cal_str));
			System.out.println( "计算结果：" + result_str );
